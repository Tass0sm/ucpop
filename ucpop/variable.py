...
class Bindings:
    ...
    def union(self, new_variables: list[Var] = [], new_constraints: Unifier = []) -> "Bindings":
        ...
        return Bindings(nodes=frozendict(mutable_nodes), unbound=frozenset(mutable_unbound))
...
class DisjunctiveBindings(Bindings):
    ...
    @classmethod
    def empty(cls):
        return DisjunctiveBindings(nodes=frozendict(), pending_disjunctions=frozenset(), unbound=frozenset())
