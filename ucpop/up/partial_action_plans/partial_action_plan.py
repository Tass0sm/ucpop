...
class PartialActionInstance(ActionInstance):
    """
    ...
    def __init__(
        ...
        for param, assigned_value in zip(action.parameters, self._params):
            ...
            if not (assigned_value.is_constant() or assigned_value.is_variable_exp()):
                raise UPTypeError(
                    f"An ActionInstance parameter must be a constant or variable: {assigned_value} is not."
                )
